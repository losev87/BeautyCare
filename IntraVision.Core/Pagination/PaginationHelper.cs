...
namespace IntraVision.Core.Pagination
{
	...
	public static class PaginationHelper
	{
		...
		{
            return source.AsPagination(pageNumber, LazyPagination<T>.DefaultPageSize, LazyPagination<T>.DefaultPagesOnPage);
		}
		...
		/// <returns>An IPagination of T</returns>
		public static IPagination<T> AsPagination<T>(this IEnumerable<T> source, int pageNumber, int pageSize, int pagesOnPage)
		{
			if(pageNumber < 1)
			{
			...
			return new LazyPagination<T>(source.AsQueryable(), pageNumber, pageSize, pagesOnPage);
		}
	}
}
