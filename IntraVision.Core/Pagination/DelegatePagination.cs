...
namespace IntraVision.Core.Pagination
{
	...
	public class DelegatePagination<T> : IPagination<T>
	{
		...
		public DelegatePagination(DoQueryDelegate<T> method, int pageNumber, int pageSize, int pagesOnPage, object additionalArgs)
		{
			DoQuery = method;
			_pageNumber = pageNumber;
			_pageSize = pageSize;
            _additionalArgs = additionalArgs;
            _pagesOnPage = pagesOnPage;
		}
        ...
        public int PagesOnPage
        {
            get { return _pagesOnPage; }
            set
            {
                if (value != _pagesOnPage)
                {
                    _doRefresh = true;
                    _pagesOnPage = value;
                }
        ...
        public bool HasNextN
        {
            get
            {
                TryExecuteQuery();
                return (PageNumber % PagesOnPage != 0 ? (PageNumber / PagesOnPage + 1) * PagesOnPage : (PageNumber / PagesOnPage) * PagesOnPage) < TotalPages;
            }
        }

        public bool HasPreviousN
        {
            get
            {
                TryExecuteQuery();
                return PageNumber > PagesOnPage;
            }
        }
	}
}