...
namespace IntraVision.Core.Pagination
{
	...
	public class LazyPagination<T> : IPagination<T>
	{
        ...
        public const int DefaultPagesOnPage = 20;
		...
		public LazyPagination(IQueryable<T> query, int pageNumber, int pageSize, int pagesOnPage)
		{
		...
		protected void TryExecuteQuery()
		{
            ...
            if (numberToSkip > totalItems || PageSize == int.MaxValue) PageNumber = 1;
        ...
        {
            get { return (PageNumber % PagesOnPage != 0 ? (PageNumber / PagesOnPage + 1) * PagesOnPage : (PageNumber / PagesOnPage) * PagesOnPage) < TotalPages; }
        }

        public bool HasPreviousN
        {
            get { return PageNumber > PagesOnPage; }
        }
	}
